...
router.post("/create", (req, res) => {
  ...
  router.post("/get_member_lists_of_user",  (req, res) => {
    ...
    lists.find({ "members.user_id" :  user_id  })
    .then(list => {
      if (!list) {
        console.log("no list");
        return res.status(404).json({ msg: "no list with this list name" });
  ...
  router.post("/subscribe",  (req, res) => {
    ...
    lists.findOne({ list_Name, "subscribers.username": { $ne: username} })
    .then(list => {
      if (!list) {
        console.log("no list");
        return res.status(404).json({ msg: "no list with this list name" });
      ...
      console.log("After adding the subscribers of the list: ", list.subscribers);
  ...
  router.post("/get_subscribed_lists",  (req, res) => {
    ...
    lists.find({ "subscribers.user_id" :  user_id  })
    .then(list => {
      if (!list) {
        console.log("no list");
